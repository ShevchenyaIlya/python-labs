...
class CustomHTTPRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if path_components[0] == "posts" and path_components_len <= 2:
            filename = generate_filename()
            if path_components_len == 2:
                unique_id = path_components[1]
                if self._get_single_post(filename, unique_id):
                    return
            else:
                self._get_all_posts(filename)
                return
    ...
    def do_POST(self) -> None:
        ...
        if path_components[0] == "posts" and len(path_components) == 1:
            filename = generate_filename()
    ...
    def _get_single_post(self, filename: str, unique_id: str) -> bool:
    ...
    def _get_all_posts(self, filename: str) -> None:
        file_content = []
        with open(filename, "r") as file:
            for line in file:
...
def parse_command_line_arguments() -> int:
...
if __name__ == '__main__':
    port_number = parse_command_line_arguments()
